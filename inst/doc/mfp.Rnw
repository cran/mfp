...
f <- mfp(Surv(rfst, cens) ~ strata(htreat)+age+fp(tumsize)+fp(posnodal)+fp(prm)+fp(esm)
        +menostat+tumgrad, family = cox, data = GBSG, verbose=T)
@

After three cycles the final model is selected. 
None of the possible input variables were excluded from the model.
Only for variables \texttt{prm} and \texttt{posnodal} nonlinear transformations were chosen. 
Prescaling was used for esm, prm, tumsize and posnodal.
